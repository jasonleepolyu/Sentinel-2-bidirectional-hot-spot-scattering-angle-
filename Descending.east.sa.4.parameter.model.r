...
if (!exists("cove_data"))  cove_data <- read.table(file="./COVE.data.2016.v3.txt", header=TRUE, sep="\t")
 ...
 modeled_east_lat <- Clat - 2*asin( cos(va*pi/180)*half_orbit/2/earthR )*180/pi ;
 
 small_r <- earthR*sin((90-abs(modeled_east_lat))*pi/180);

 delta_lon <- 2*asin( -1 * sin(va*pi/180)*half_orbit/2/small_r )*180/pi;
 
 model_east_lt <- model_cent_lt + delta_lon/15;
 ...
 # write.table(SA, "./COVE.data.2016.SA.reference.descending.Eastern.txt", sep="\t", row.names = FALSE, col.names = FALSE)
