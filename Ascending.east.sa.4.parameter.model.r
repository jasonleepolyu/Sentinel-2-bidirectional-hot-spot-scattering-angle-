############################################################################################################ 
### This code is used to calculate the Sentinel-2 scattering angles on COVE ascending orbit eastern edges using 4-parameter model (without time and location info provided by COVE KML files)
...
# To run: source("Ascending.east.sa.4.parameter.model.r")
...
###  inclination 
 ...
 ### For Eastern side of COVE acquisition
 vaa <- 3*pi/2 - j  
 va <- vaa * 180 / pi
 ...
 ### For Eastern side of ascending 
 modeled_east_lat <- 2 * asin( cos((180-98.62)/180*pi)/cos(Clat/180*pi)*half_orbit/2/earthR )*180/pi + Clat ;
 
 small_r <- earthR*sin((90-abs(modeled_east_lat))*pi/180);
 delta_lon <- 2*asin(-1*sin((180-va)*pi/180)*half_orbit/2/small_r)*180/pi;  ## -1* ZB
 modeled_east_lt <- model_cent_lt + delta_lon/15;
 ...
 dLocalMeanSiderealTime <- (6.6974243242 + 0.0657098283 * dElapsedJulianDays + modeled_east_lt ) * 15 * rad;  ###   local time on the eastern edge 
